...
function formatVigencia(vigencia_hasta) {
  if (!vigencia_hasta) return '&mdash;'
  try {
    return new Date(vigencia_hasta + 'T00:00:00').toLocaleDateString('es-PE', {
...
function fotocheckHTML(c, logoBase64) {
  ...
  const vigencia  = formatVigencia(c.vigencia_hasta)
  ...
  const respNombre   = esc(`${c.nombre  || ''} ${c.apellido  || ''}`.trim())
...
">
  ...
  <div style="flex-shrink:0;">
    ...
    <!-- Responsable -->
    <div style="background:#f7f9fc;padding:0.8mm 3mm;display:flex;flex-direction:column;gap:0.2mm;">
      <span style="font-size:3.8pt;color:#999;letter-spacing:0.4mm;text-transform:uppercase;font-weight:bold;line-height:1.2;">Responsable</span>
      <span style="font-size:6.5pt;color:${MARINO};font-weight:bold;line-height:1.3;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${respNombre}</span>
    </div>
    <!-- Parentesco + Vigencia -->
    <div style="display:flex;">
      <div style="flex:1;padding:0.8mm 3mm;display:flex;flex-direction:column;gap:0.2mm;">
        ...
        <span style="font-size:6.5pt;color:${MARINO};font-weight:bold;line-height:1.3;">${esc(c.parentesco)}</span>
      </div>
      <div style="width:0.3mm;background:#eee;flex-shrink:0;"></div>
      <div style="flex:1;padding:0.8mm 3mm;display:flex;flex-direction:column;gap:0.2mm;">
        <span style="font-size:3.8pt;color:#999;letter-spacing:0.4mm;text-transform:uppercase;font-weight:bold;line-height:1.2;">Vigencia</span>
        <span style="font-size:6.5pt;color:${MARINO};font-weight:bold;line-height:1.3;">${vigencia}</span>
      </div>
    </div>
  </div>
  ...
  ">
    <div style="display:flex;align-items:center;gap:1mm;">
      <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="width:3mm;height:3mm;display:inline-block;">
        ...
        <path d="M8 11V7a4 4 0 0 1 8 0v4" stroke="${DORADO}" stroke-width="2" stroke-linecap="round" fill="none"/>
        <circle cx="12" cy="16" r="1.5" fill="${MARINO}"/>
      </svg>
      <span style="color:${DORADO};font-size:4.5pt;font-weight:bold;letter-spacing:0.3mm;">RECOJO SEGURO</span>
    </div>
...
/**
 * Abre una ventana con los fotochecks de recojo listos para imprimir.
 * Layout A4 portrait: 2 columnas × 3 filas = 6 fotochecks por hoja.
 * @param {Array} fotochecks — cada item: { nombre, apellido, parentesco, vigencia_hasta, qrBase64, estudiante: {...} }
 */
export async function imprimirFotochecks(fotochecks) {
  ...
  win.document.write(`<!DOCTYPE html><html><body style="font-family:Arial;text-align:center;padding:40px;color:#555;">
    <p>Preparando fotochecks de recojo...</p></body></html>`)
...
<head>
  <meta charset="UTF-8">
  <title>Fotochecks Recojo &mdash; CEAUNE</title>
  <style>
...
<body>
  ...
  <p class="header-impresion">
    CEAUNE &mdash; Fotochecks de Recojo &mdash;
    ${fotochecks.length} fotocheck${fotochecks.length !== 1 ? 's' : ''} &mdash; ${esc(fecha)}
  </p>

  <div class="grid-fotochecks">
    ${fotochecks.map(c => `<div class="fotocheck-wrapper">${fotocheckHTML(c, logoBase64)}</div>`).join('\n    ')}
  </div>
